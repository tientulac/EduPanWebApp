...
@Component({
  selector: 'app-phuong-phap-nghien-cuu',
  templateUrl: './phuong-phap-nghien-cuu.component.html',
  styleUrls: ['./phuong-phap-nghien-cuu.component.scss']
})
export class PhuongPhapNghienCuuComponent implements OnInit {
  ...
  Insert = new FormGroup({
    Ten_phuong_phap: new FormControl('', [Validators.required]),
  ...
  constructor(
    ...
    private PhuongPhapNghienCuu: PhuongPhapNghienCuuService
  ) { }
  ...
  GetAll() {
    this.PhuongPhapNghienCuu.Load_List(this.Token)
      .subscribe(z => {
  ...
  open(content, sizm, type, Data) {
    this.selected_ID = Data.ID_phuong_phap;
    ...
    this.modalService
      ...
      if(type=="Update") {
        ...
        this.Insert.patchValue({
          Ten_phuong_phap: Data.Ten_phuong_phap,
  ...
  Delete() {
    this.PhuongPhapNghienCuu.Delete(this.selected_ID, this.Token)
      .subscribe(z => {
  ...
  onSubmit() {
    ...
    let req = {
      ID_phuong_phap: this.selected_ID,
      Ten_phuong_phap: this.Insert.value.Ten_phuong_phap,
      Ngay_tao: this.Insert.value.Ngay_tao,
      Trang_thai: this.Insert.value.Trang_thai,
    }
    this.spinner.show();
    if(this.checkInsert){
      this.PhuongPhapNghienCuu.Insert(req,this.Token).subscribe((res)=>{
        if (res.Status == 2) {
      ...
      this.PhuongPhapNghienCuu.Update(req,this.Token).subscribe((res)=>{
        if (res.Status == 2) {
          this.toastr.warning(res.Message);
          this.spinner.hide();
        } else if (res.Status == 1) {
